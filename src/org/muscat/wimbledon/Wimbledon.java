...
public class Wimbledon implements ScoresHandler {
  ...
  public void handle(final Scores scores) {
    ...
    for (final MatchScore match : scores.getMatches()) {
      System.out.format("%-14s %s%25s%s %s%s%s %s%-5s%s  %-25s  %s%s%s %s%-25s%s\n",
          match.getCourt(),
          match.getWinner() == 1 ? CONSOLE_BOLD : CONSOLE_NORMAL, match.getPlayer1(), CONSOLE_NORMAL,
              ...
              match.getWinner() == 2 ? CONSOLE_BOLD : CONSOLE_NORMAL, match.getPlayer2(), CONSOLE_NORMAL
          );
    }
