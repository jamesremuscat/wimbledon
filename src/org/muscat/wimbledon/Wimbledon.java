...
public class Wimbledon implements ScoresHandler {
  ...
  public void handle(final Scores scores) {
    ...
    for (final MatchScore match : scores.getMatches()) {
      System.out.format("%-14s%25s %s%-7s%s  %-25s %-25s\n", match.getCourt(), match.getPlayer1(), CONSOLE_BOLD, match.getPointsAsString(), CONSOLE_NORMAL, match.getScoresAsString(), match.getPlayer2());
    }
